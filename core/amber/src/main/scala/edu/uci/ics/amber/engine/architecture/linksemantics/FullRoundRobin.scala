...
class FullRoundRobin(from: WorkerLayer, to: WorkerLayer, batchSize: Int) extends LinkStrategy(from, to, batchSize) {
  override def getPolicies(): Iterable[(ActorVirtualIdentity, DataSendingPolicy, Seq[ActorVirtualIdentity])] = {
    ...
    from.identifiers.map(x => (x, new RoundRobinPolicy(id, batchSize, to.identifiers), to.identifiers.toSeq))
  }
