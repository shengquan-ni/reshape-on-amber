...
import edu.uci.ics.amber.engine.architecture.linksemantics.{AllToOne, FullRoundRobin, HashBasedShuffle, LinkStrategy, OneToOne, RangeBasedShuffle}
...
class Workflow(
  ...
  def getOperatorIdentity(opExecConfig: OpExecConfig): OperatorIdentity = {
    var opId: OperatorIdentity = null
    operators.keys.foreach(k => {
      if (k == opExecConfig.getOperatorIdentity()) { opId = k }
    })
    opId
  }
